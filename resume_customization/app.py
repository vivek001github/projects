...
def save_to_docx(customized_resume):
    ...
    for line in lines:
        ...
        if not name_added:
            ...
            # Bold "Technical Skills:" with size 11
            skills_para = doc.add_paragraph()
            skills_run = skills_para.add_run(line)
            skills_run.font.size = Pt(11)
            skills_run.bold = True
            skills_para.paragraph_format.space_after = Pt(0)
        elif line == "Software Skills:":
            # Bold "Software Skills:" with size 11
            software_para = doc.add_paragraph()
            software_run = software_para.add_run(line)
            software_run.font.size = Pt(11)
            software_run.bold = True
            software_para.paragraph_format.space_after = Pt(0)
        elif line == "Soft Skills:":
            # Bold "Soft Skills:" with size 11
            soft_skills_para = doc.add_paragraph()
            soft_skills_run = soft_skills_para.add_run(line)
            soft_skills_run.font.size = Pt(11)
            soft_skills_run.bold = True
            soft_skills_para.paragraph_format.space_after = Pt(0)
        else:
            # Regular content
